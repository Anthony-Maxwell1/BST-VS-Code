...
function ensureStudioAlive() {
    if (!studioHwnd)
        throw new Error("Studio HWND missing");
}
export function above() {
    ensureStudioAlive();
    SetWindowPos(studioHwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
}
export function below() {
    ensureStudioAlive();
    SetWindowPos(studioHwnd, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
}
export function position(x, y, w, h) {
    ensureStudioAlive();
    SetWindowPos(studioHwnd, HWND_TOP, x, y, w, h, SWP_NOACTIVATE | SWP_SHOWWINDOW);
}
...
export function alignStudioToEditor() {
    ...
    const editorHeight = windowHeight - TITLEBAR_HEIGHT - TABBAR_HEIGHT - STATUSBAR_HEIGHT;
